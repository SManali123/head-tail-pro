...
int main(int argc, char *argv[]) {	
		...
		switch(find) {
			case 0:
				...
				if(fp==NULL) {
					...
					printf("File not found, check the error 2 %d\n",errno);
			...
			case 2:
				...
				fp = fopen(filename,"r");
				if(fp == NULL) {
					printf("File not found, check the error 4 %d\n",errno);
	      				return -1;
				}
				head(n,fp);
				return 0;
			...
			case 3:
				...
				fp = fopen(filename,"r");
				if(fp == NULL) {
					printf("File not found, check the error 5 %d\n",errno);
	      				return -1;
				}
				if(n > 0) {
				head(n,fp);
	...
	if(argc == 3) {         
		...
		if(fp==NULL && fp1==NULL ) {
			printf("File not found, check the error 6 %d\n",errno);
	      		return -1;
		}
		printf("<===%s===>\n",filename);
		head(n,fp);
